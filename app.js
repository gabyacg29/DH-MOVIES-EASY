...
//Variables para movies

let totalPelis = movies.length;
// cantidad de películas
let titulosPelis = movies.map(movie => movie.title);
// me devuelve un arreglo con solo los titulos
let pelisOrdenadas = titulosPelis.sort().join('\n');
// ordena yconvierte el arreglo en un string y las coloca una debajo de otra
let listadoPelis = movies.map(movie => [movie.title, movie.overview]);
//me devuelve un array con el titulo de las peliculas y el overview
let listadoPeliculas = listadoPelis.map(movie => movie.join('\n\n')).join('\n\n');
// me transforma el arreglo listado de pelis en string
let masVotadas = movies.filter(movie => movie.vote_average >= 7);
// solo devuelve los que pasan la condición
let puntajes = masVotadas.map(movie => movie.vote_average);
// me devuelve un arreglo con las peliculas mas votadas
let sumaPuntajes = puntajes.reduce((anterior, actual) => actual += anterior);
//con el metodo reduce() voy acumulando la suma de los puntajes 
let puntajePromedio = (sumaPuntajes / puntajes.length).toFixed(1);
//me devuelve el puntaje promedio y se usa tofixed(1)para que me devuelva un string con solo un decimal
...
// me devuelve un arreglo con todos los datos de las peliculas

let listaMasVotadas = datosMasVotadas.map(movie => movie.join('\n')).join('\n\n');
// me transforma el arreglo de datosMasVotada a string
//Hasta aqui las variables 
...
// me devuelve el total de faqs
let listadoFaqs = faqs.map(faqs => [faqs.faq_title, faqs.faq_answer]);
// me devuelve un arreglo con los titulos de las preguntas y las respuestas
let listadoFaqsFinal = listadoFaqs.map(faqs => faqs.join('\n')).join('\n\n');
...
//Variables para theaters
/*let totalSalas = theaters.reduce(function(prev, cur) {
    return prev + cur.total_rooms;
}, 0);*/

let arrayTotalSalas = theaters.map(theaters => theaters.total_rooms);
// me devuelve un arreglo con la cantidad de salas
let totalSalas = arrayTotalSalas.reduce((anterior, actual) => actual += anterior);
//con el metodo reduce() voy acumulando la suma de los puntajes del arreglo de arraytotalSalas

let listadoSalas = theaters.map(theaters => [theaters.name, theaters.address, theaters.description]);
// me devuelve un arreglo con los datos de las salas
let listadoSalasFinal = listadoSalas.map(theaters => theaters.join('\n')).join('\n\n');
...
http.createServer((req, res) => {
    ...
    switch (req.url) {
        ...
        case '/mas-votadas':
            res.end(`
                      MAS VOTADAS
                      Total de películas mas votadas: ${masVotadas.length}
        ...
        case '/sucursales':
            res.end(`       SUCURSALES
                            Nuestras Salas.Total de salas: ${totalSalas}
                            Listado de salas:
                            \n ${listadoSalasFinal}
                            `);
            break;
